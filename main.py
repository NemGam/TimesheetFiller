...
def get_current_pay_period(wb : openpyxl.Workbook) -> Worksheet:
    ...
    for sheet in wb:
        ...
        if not final_sheet and datetime.datetime.strptime(sheet.title, "%m_%d_%Y").date() >= input_date:
            final_sheet = sheet
...
def convert_to_PDF(sheet_to_convert_ID : int):
    ...
    try:
        ...
        print("Error occured. No conversion happened.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(error)
        print("Please put 'Timesheet.xlsx' in the same directory as this file.")
    ...
    print(f"File successfully created. ({CURRENT_DIRECTORY.joinpath(f'{FULL_NAME} {activeSheet.title}.xlsx')})")
    input("Press Enter to exit...")
